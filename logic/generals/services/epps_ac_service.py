...
class EPPSACService():
    def __init__(self, lazy: bool = False):
        self._cache: list[EPPSACUser] = []
        self.folder_path = DATA_PATH
        
        self.file_enum: FileName = FileName.EPPS_AC
    ...
    def cargar_datos(self) -> None:
        ...
        try:
            ...
            print(f"Usuarios EPPS AC ({self.file_enum.name}) | Total en cache: {len(self._cache)}")

        except Exception as e:
            print(f"Error cargando datos desde {self.path_file}: {e}")
    
    def get_all(self) -> list[EPPSACUser]:
        return self._cache
